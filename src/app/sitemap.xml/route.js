...
import { applications, categories, products, serviceLocations } from "@/data/data";
...
export async function GET() {
    const baseUrl = "https://advertisingleddisplay.in";
    ...
    const categoriesUrls = categories.map((product) => `
    <url>
    ...
    const applicationsUrls = applications.map((product) => `
    <url>
    ...
    const blogUrls = blogs.filter((blog) => blog?.permalink).map((blog) => `
    <url>
